...
const UserList = () => {
    ...
    //table columns
    const COLUMNS = [
        { label: 'Name', renderCell: (user: any) => <div className='text-gray-700 text-xs'>{user.name}</div> },
        { label: 'Email', renderCell: (user: any) => user.email },
        { label: 'Phone', renderCell: (user: any) => user.phone },
        { label: 'User Type', renderCell: (user: any) => <div>Account-{user.roles.role}</div> },
            ...
            <div>
                ...
                <div className='flex items-start justify-between bg-white p-5 rounded-b-md'>
                    ...
                    <span className='w-1/2 flex items-start justify-end gap-2'>
                        ...
                        >
                            {pagination.state.getTotalPages(users)}</button>
                        <button
                            className={`text-xs px-2 py-1 text-gray-600 border border-1 border-gray-300 rounded-md font-semibold ${pagination.state.page >= pagination.state.getTotalPages(users) - 1 ? 'pointer-events-none opacity-50' : ''}`}
                            onClick={() => {
                                if (pagination.state.page < pagination.state.getTotalPages(users) - 1) {
                                    pagination.fns.onSetPage(pageIndex + 1)
                                    setPageIndex(pageIndex + 1);
                                }
                            }}
